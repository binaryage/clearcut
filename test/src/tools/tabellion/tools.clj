...
(defn gen-devtools-if-needed []
  (if-not (= (env :tabellion-elide-devtools) "1")
    `(under-chrome
...
(defn fetch-arena-source [ns-name]
  (let [file-name (str (namespace-munge (last (string/split ns-name #"\."))) ".cljs")
        file (io/file "test/src/arena/tabellion/arena" file-name)]
    (slurp file)))
...
(defmacro with-stderr-recording [recorder & body]
  ...
  `(do
     (tabellion.tools/start-err-recorder!)
     (let [res# (do ~@body)
           recording# (tabellion.tools/stop-err-recorder!)]
