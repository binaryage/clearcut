(ns clearcut.circus.build
  (:require [clojure.string :as string]
            ...
            [clearcut.circus.utils :refer :all]
            [clearcut.circus.config :as config]
...
(defn build-options [main variant config & [overrides]]
  ...
  (let [out (str (last (string/split main #"\.")) (if-not (empty? variant) (str "-" variant)))
        compiler-config (merge {:pseudo-names  true
                               ...
                               (if-not (empty? config)
                                 {:external-config {:clearcut/config config}}))]
...
(defn make-build [file variant & [config overrides]]
  (let [source (str "test/src/arena/clearcut/arena/" file)]
    {:source  source
     :variant variant
...
(defn get-build-name [build]
  (let [{:keys [source variant]} build]
    (str (string/replace source #"test\/src\/arena\/clearcut\/" "") (if-not (empty? variant) (str " [" variant "]")))))
...
(defn read-build-output [build]
  (let [output-path (get-in build [:options :output-to])]
    (try
      ...
      ;     https://github.com/binaryage/clearcut/tree/master/test/transcripts/expected
